...
const useFileSystemContextState = (): FileSystemContextState => {
  ...
  const deletePath = useCallback(
    async (path: string, permanent = false): Promise<boolean> => {
      const isAlreadyInRecycleBin = path.startsWith("/System/Recycle Bin");

      if (permanent || isAlreadyInRecycleBin) {
        let deleted = false;
        ...
        try {
          ...
          if ((error as ApiError).code === "EISDIR") {
            ...
            await Promise.all(
              dirContents.map((entry) => deletePath(join(path, entry), true))
            );
        ...
        if (!(await exists("/System/Recycle Bin"))) {
          await mkdirRecursive("/System/Recycle Bin");
        }

        let destPath = join("/System/Recycle Bin", basename(path));
        let iteration = 0;
        ...
        const moved = await rename(path, destPath);
        if (moved) {
          await updateFolder(dirname(path), undefined, basename(path));
          await updateFolder("/System/Recycle Bin", basename(destPath));
        }
        return moved;
      }
    },
